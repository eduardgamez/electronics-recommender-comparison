...
* **Flujo de Datos:**
  * **Origen:** Dataset real de Amazon Electronics con interacciones de usuario. *Procesado mediante `dataset/process_data.ipynb`*.
...
> **Nota para los Notebooks (`.ipynb`):** Al ejecutar `pip install -r requirements.txt`, se instalará automáticamente `ipykernel` para habilitar que los notebooks puedan utilizar las mismas dependencias. Selecciona tu entorno `venv` en la esquina superior derecha de VS Code ("Select Kernel").
